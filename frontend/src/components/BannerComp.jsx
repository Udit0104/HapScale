...
const BannerComp = () => {
    return (
        <div id='speciality' className='flex flex-col items-center justify-center text-center gap-6 py-16 text-[#fff] w-full'>
            ...
            <div className='flex justify-center items-center gap-12 pt-5 w-full'>
                {specialityData.map((item, index) => {
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-col items-center justify-center text-base cursor-pointer flex-shrink-0 hover:translate-y-[-10px] transition-all duration-500 mb-4 ${
                                index === 0 ? 'animate-light-pop' : ''
                            }`}
                            key={index}
                        >
                            <img className='w-20 sm:w-32 mb-2' src={item.image} alt={item.speciality} />
                            <p className="text-lg">{item.speciality}</p> {/* Increased size */}
                        </Link>
                    );
                })}
            </div>
